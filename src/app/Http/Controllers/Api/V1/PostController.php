...
class PostController extends Controller
{
    ...
    {
        $this->appid = env('WEATHER_APPID');
        $this->url_weather = env('WEATHER_URL');
    }
    ...
    */
    private function _get_temperature_api($city = NULL, $lat = NULL, $lon = NULL)
    {
        ...
        {
            $url = ($city != NULL) ?
                $this->url_weather . "q={$city}&APPID={$this->appid}&lang=es"
                : $this->url_weather . "lat={$lat}&lon={$lon}&APPID={$this->appid}&lang=es";
    ...
    */
    ...
    private function _get_playlist($genre)
    {
        ...
        $spotify->genreMusic = $genre;
        $tracks = $spotify->get_playlist();

        $array_songs = [];

        foreach ($tracks->tracks->items as $value)
        {
